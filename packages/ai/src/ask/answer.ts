...
export async function generateAskAnswer(input: { model: LanguageModel; query: string; tickets: CompactTicket[] }): Promise<string> {
  ...
  const { system, prompt } = buildAnswerPrompt({ query, tickets });
