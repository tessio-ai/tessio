...
export async function planQuery(input: { model: LanguageModel; query: string; now: string }): Promise<AskPlan> {
  // Guardrail: strip PII from the user's question before it reaches OpenAI.
  const { system, prompt } = buildPlanPrompt({ query: redactPii(input.query), now: input.now });
